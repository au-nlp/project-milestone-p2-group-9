...
|-> ``p2-readme.md`` — The README.md file from the P2 milestone <br>
...
- P3 introduces silver summaries generated by LLM, including both global summaries and role-specific summaries, and uses them as the core intermediate representations for downstream analysis, rather than relying on manually annotated gold summaries. This design is closer to real-world application settings, but inevitably introduces noise from the generation process.
- P3 does not strictly reproduce the methods described in the original project plan. As the codebase of the referenced papers is relatively outdated and offers limited practical value for direct reuse, we only draw inspiration at the methodological level. The overall implementation is newly designed and written to better fit the current data and task requirements. And we narrow the summarization pipeline to a **two-stage extractive–abstractive framework**, removing explicit topic modeling and contrastive learning components
- For conflict detection, P3 does not employ a supervised stance classification approach. Instead, viewpoint conflict is formulated as a natural language inference (NLI) problem, using a pretrained NLI model to infer semantic relationships between the role-specific summaries of the Host and the Guest
- Although retrieval-augmented generation (RAG) was discussed in the P2 plan, the current P3 implementation does not include an explicit retrieval module. Instead, it adopts a summary-centric pipeline, where summaries are generated first and then used for downstream analysis
